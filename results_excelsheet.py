...
with open('results.txt') as file:
    for line in file:
            if "1. time taken to handle the router port request in agent cloud" in line:
                x=re.split("1. time taken to handle the router port request in agent cloud", line)
                values.append(x[1])
...
worksheet.write('C1', 'time_taken_for_duplicate_router rc', bold)
worksheet.write('D1', 'Time taken to duplicate router port rc', bold)
worksheet.write('E1', 'time_taken to create ipsec, ike and vpn service', bold)
worksheet.write('F1', 'time_taken to establish s2s connectivity', bold)
worksheet.write('G1', 'total*.time taken to create extra routers', bold)
worksheet.write('H1', 'time taken for creating s2s', bold)
row = 1
col = 0
for item in (x):
